...
class AbstractControllerTest extends PHPUnit_Framework_TestCase{
    ...
    public function testHandleRoute(){
        ...
        $this->_mock->expects($this->once())->method('render')->willReturnCallback(function($request, $response) use($requestMock, $responseMock){
            $this->assertSame($request,$requestMock);
            $this->assertSame($response,$responseMock);
        });
        $this->assertSame($responseMock,$this->_mock->handleRoute($requestMock, $responseMock));
    }
